...
public class BubbleTeaFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        root = inflater.inflate(R.layout.fragment_bubble_tea, container, false);
        setControl();
        return root;
    }
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                    if (snapshot.child("id_store").getValue() != null) {
                        if (snapshot.child("id_menu").getValue().toString().equals("002") && snapshot.child("id_store").getValue().toString().equals(idStore)) {
                            addProduct(snapshot);
                        }
                    }
                }
